...
def main(file, resource):
    if not file and not resource:
        logger.error(
            "No any file or resource provided.",
            extra={
                "type_of_resource": "No resource",
                "name_of_resource": "No resource",
            },
        )
        sys.exit(1)
        ...
        for f in file:
            pool.apply_async(
                ...
                (
                    f,
                    ResourceMeta(
                        type=ResourcesTypes.FILE,
                        name=f, execution_date=execution_date
                    ),
                ),
            )
...
def process_file(file_path, metadata: ResourceMeta = None):
    ...
    execution_logger.info(
        f"Start processing text from file [{file_path}]",
        extra=_metadata_to_extra_logs(metadata),
    )
    if os.path.exists(file_path):
        try:
            with open(file_path) as file:
                run_analysis(file.read(), metadata=metadata)
        except Exception as e:
            execution_logger.error(
                f"Error when opening the file: {e}",
                extra=_metadata_to_extra_logs(metadata),
            )
            return
    else:
        execution_logger.error(
            f"Path [{file_path}] does not exists.",
            extra=_metadata_to_extra_logs(metadata),
        )
...
def process_resource(resource, metadata: ResourceMeta = None):
    ...
    """
    execution_logger.info(
        f"Start processing text from resource [{resource}]",
        extra=_metadata_to_extra_logs(metadata),
    )
    try:
        response = requests.get(resource)
        if response.status_code == 200:
            run_analysis(response.text, metadata=metadata)
        else:
            execution_logger.error(
                f"Got invalid response from url: [{resource}]",
                extra=_metadata_to_extra_logs(metadata),
            )
            return
    except Exception as e:
        execution_logger.error(
            f"Error processing the request: {e}, url: [{resource}]",
            extra=_metadata_to_extra_logs(metadata),
        )
...
def run_analysis(text: str, metadata: ResourceMeta = None):
    ...
    with open(
        os.path.join(
            ANALYSE_RESULTS_PATH,
            f"analysis_results_{datetime.now().isoformat()}.txt"
        ),
        "w",
    ) as file:
        if metadata:
    ...
    execution_logger.info(
        "Execution finished.", extra=_metadata_to_extra_logs(metadata)
    )
