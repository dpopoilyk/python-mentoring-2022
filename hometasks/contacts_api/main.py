...
async def get_contacts(
    ...
    filters: BaseModel = Depends(
        create_model(
            "Filters",
            **{field.value: (Optional[str], None) for field in ContactFields}
        )
    ),
    limit: int = Query(5),
    offset: int = Query(0),
    ...
    response = ResponseEntity(
        ...
        meta=ResponseMeta(
            fields=list(
                contacts[0].dict().keys()
            ) if len(contacts) > 0 else None,
...
async def update_contact(
    ...
    try:
        contact_to_update = (
            await repository.get_contacts(filters={"uid": uid})
        )[0]
    except IndexError:
        raise NotFoundException(f'Contact wit uid: "{uid}" not found')

    contact_to_update, updated_fields = await repository.update_contact(
