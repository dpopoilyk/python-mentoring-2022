...
from hometasks.announcements_api.core.entities import OrderDirection, QueryFilter
...
from hometasks.announcements_api.core.depends import get_announcements_repository
from hometasks.announcements_api.core.responses import ResponseEntity, ResponseMeta
...
async def create_announcement(
        ...
        request: Request,
        repository: AnnouncementsRepository = Depends(get_announcements_repository)
):
...
async def get_contacts(
        request: Request,
        repository: AnnouncementsRepository = Depends(get_announcements_repository),
        order_by: Optional[AnnouncementFields] = Query(AnnouncementFields.create_date),
        order_direction: Optional[OrderDirection] = Query(OrderDirection.ASC),
        filters: BaseModel = Depends(create_model('Filters', **{field.value: (Optional[str], None) for field in AnnouncementFields})),
        limit: int = Query(5),
        offset: int = Query(0),
):
    ...
    response = ResponseEntity(
        ...
        meta=ResponseMeta(
            fields=list(announcements[0].dict().keys()) if len(announcements) > 0 else None,
...
async def update_announcement(
    ...
    try:
        announcement_to_update = (await repository.get_announcements(filters={'uid': uid}))[0]
    except IndexError:
        raise NotFoundException(f'Announcement with uid: "{uid}" not found')

    announcement_to_update, updated_fields = await repository.update_announcement(announcement, announcement_to_update)
...
async def delete_announcement(
    ...
    try:
        announcement_to_delete = (await repository.get_announcements(filters={'uid': uid}))[0]
    except IndexError:
        raise NotFoundException(f'Announcement with uid: "{uid}" not found')
